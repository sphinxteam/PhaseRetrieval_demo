...
for gamma in gammas:
    ...
    alpha_WR = 1./(1.+gamma)
    alpha_PR = min(2.,2./gamma)
    alphas = np.linspace(0.01, 90*alpha_WR/100, 8, endpoint = False)
    alphas = np.concatenate((alphas, np.linspace(90*alpha_WR/100, 110*alpha_WR/100, 15 , endpoint = False))) #Around the WR transition
    #Now around alpha_PR 
    alphas = np.concatenate((alphas, np.linspace(110*alpha_WR/100, 90*alpha_PR/100, 30, endpoint = False)))
    alphas = np.concatenate((alphas,np.linspace(90*alpha_PR/100,110*alpha_PR/100,20, endpoint = False)))
    alphas = np.concatenate((alphas,np.linspace(110*alpha_PR/100,130*alpha_PR/100,2)))
