...
from Custom_Class.Custom_Priors import ComplexGaussianPrior_ZeroIm
...
def mse_gvamp(alpha, N = 8000, verbosity = 0, max_iter = 1000):
    ...
    prior_z = ComplexGaussianPrior_ZeroIm(size=size, mean=mean)
    prior_z_no_mean = ComplexGaussianPrior_ZeroIm(size=size, mean=0.)
    M = int(alpha * N)
...
def run_alpha(alpha, max_iter = 1000):
    ...
    Nb_averages = 5 #Number of runs on which we average and take an error bar #FIXME
    if alpha < 0.49:
        Nb_averages = 2
    ...
    filename = "Data/tmp/results_gvamp_complex_matrix_real_signal_alpha_"+str(alpha)+".pkl"
...
max_iter = 1000
print("This is GVAMP for complex matrix+real signal")

#The lists of alphas
alphas = np.linspace(0.2, 0.49, 2 , endpoint = False)
alphas = np.concatenate((alphas, np.linspace(0.49, 1.0, 15, endpoint = False)))
alphas = np.concatenate((alphas,np.linspace(1.0,1.05, 5 , endpoint = False)))
...
filename = "Data/results_gvamp_complex_matrix_real_signal.pkl"
outfile = open(filename,'wb')
