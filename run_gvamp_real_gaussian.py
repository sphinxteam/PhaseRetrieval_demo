...
def mse_gvamp(alpha, N = 8000, verbosity = 0, max_iter = 1000):
    ...
    M = int(alpha * N)
    A = GaussianEnsemble(M,N).generate() # generate sensing matrix A
    model_teacher = prior_z_no_mean @ V(id="z") @ LinearChannel(A, name="A") @ V(id="Az") @ AbsChannel() @ O(id="y") # build model
    model_student = prior_z @ V(id="z") @ LinearChannel(A, name="A") @ V(id="Az") @ AbsChannel() @ O(id="y") # build model
...
def run_alpha(alpha, max_iter = 1000):
    ...
    filename = "Data/tmp/results_gvamp_real_gaussian_alpha_"+str(alpha)+".pkl"
...
max_iter = 1000
print("This is the GVAMP for real Gaussian")
alphas = np.linspace(0.2, 0.49, 2 , endpoint = False)
alphas = np.concatenate((alphas, np.linspace(0.49, 1.0, 15, endpoint = False)))
alphas = np.concatenate((alphas,np.linspace(1.0,1.15, 5 , endpoint = False)))
alphas = np.concatenate((alphas,np.linspace(1.15,1.5,3)))
...
output = {'alphas':alphas,
        'mses_mean_all':mses_mean_all,'mses_std_all':mses_std_all,'mses_mean_converged_only':mses_mean_converged_only,'mses_std_converged_only':mses_std_converged_only}
